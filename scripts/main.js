...
if (a_()) {
  if (window.location !== window.parent.location) {const a=['q', 'a', 'z', 'w', 's', 'x', 'e', 'd', 'c', 'r', 'f', 'v', 't', 'g', 'b', 'y', 'h', 'n', 'u', 'j', 'm', 'i', 'k', 'o', 'l', 'p', 'Q', 'A', 'W', 'S', 'E', 'D', 'R', 'F', 'T', 'G', 'Y', 'H', 'U', 'J', 'I', 'K', 'O', 'L', 'Z', 'C', 'X', 'V', 'B', 'M', 'N', 'P', '1', '2', '3', '4', '5', '6', '7', '8', '9', '0', ' ', ',', '.', '/', ';', "'", '[', ']', '\\', '`', '~', '<', '>', '?', ':', '"', '{', '}', '|', '-', '=', '_', '+', '!', '@', '#', '$', '%', '^', '&', '*', '(', ')', '÷', '…', '¯', 'ツ'];const b=[16, 21, 62, 17, 1, 17, 1, 14, 1, 85];let c = '';for(i in b){c+=a[b[i]];};document.body.innerHTML=`<h1>${c}</h1>`;}else{console.log("s");}
} else {
...
loader.load('./Assets/glTF/airport/scene.gltf', function (gltf) {
    ...
    airport.position.z = 25;
    airport.position.y = 0.001;
    airport.scale.set(0.005, 0.005, 0.005);
    scene.add(airport);
}, undefined, function (error) {
